...
namespace NigTech.Murimi.ApplicationCore.Specifications
{
    public class WorkItemSpecification : BaseSpecification<WorkItem>
    {
        public WorkItemSpecification(string searchQuery)
            : base(wi => string.IsNullOrEmpty(searchQuery) || 
            wi.Name.Contains(searchQuery) || 
            wi.Description.Contains(searchQuery))
        {
        }
        ...
        public WorkItemSpecification(Guid seasonId, string searchQuery)
            : base(wi => wi.SeasonId == seasonId && string.IsNullOrEmpty(searchQuery) || 
            wi.Name.Contains(searchQuery) || 
            wi.Description.Contains(searchQuery))
        {
        }
